...
$('.load')[0].style.display='none'
	...
	function popup_gift(){
		// 获得奖品
			...
			if(type == 'wechat' && fishing_sex != '') {
				if(fishing_sex == sex) {
					$('.warehouse_img')[0].src = 'image/start-fishing/基友卡.png';
					$('.img1')[0].src = 'imgaes/start/框框2.png';
					$('.img2')[0].src = fishing_img;
					$('.warehouse_name')[0].innerText = fishing_name
				}
				if(fishing_sex != sex) {
					$('.warehouse_img')[0].src = 'image/start-fishing/桃花卡.png';
	...
	function addType() {
		$.post(
			'http://www.xiaohulaile.cn/web_sock/public/index/index/user_update', // 请求地址
			{
				'data': [{ // 数据data
					'type': type,
					'prize': prize,
